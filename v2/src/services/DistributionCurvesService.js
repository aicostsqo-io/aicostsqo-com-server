const { normal, logNormal, exponential, rayleigh, } = require('../scripts/utils/formuls');
const {
    chiSquareTest,
    minMaxNormalization,
} = require('../scripts/utils/goodnesOfFitTests');
const { insert } = require('./OutputVolumeService');

const _context = {
    outputPolyhedrons: require('../models/OutputPolyhedron'),
    outputMaxQs: require('../models/OutpuMaxQ'),
    outputVolumes: require('../models/OutputVolume'),
};
...
const createOutputVolumes = async (rpId) => {
    ...
    const insertResult = await insert(preprocessedData);
    return insertResult;
};
